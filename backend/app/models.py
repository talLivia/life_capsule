...
class Entity(Base):
    ...
    # Display grouping WITHIN a type, for the timeline's grouped bubbles —
    # school | higher_education | military | workplace | community | other,
    # CHECK-enforced by migration 0022. `type` alone cannot say "school":
    # three schools, the air force and a college all share type='organisation'.
    # NULL = never classified; 'other' = classified and nothing fit — the same
    # never-asked vs asked-and-unknown distinction as the *_asked_at columns,
    # and what stops a hard case being re-sent to the model on every read.
    # Assigned by period_insights (an LLM display judgement, never a fact the
    # producer stated), and only ever read for grouping.
