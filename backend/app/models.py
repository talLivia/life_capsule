...
class RawSegment(Base):
    ...
    # archive size (docs/MEDIA_GALLERY.md §1.7). Generated by period_insights,
    # lazily at read; NULL means not yet generated (retried). Stale when the
    # language no longer matches OR the transcript no longer hashes to
    # moment_title_source (migration 0024) — an in-place transcript change
    # must not leave a title naming words that are gone.
    moment_title = Column(Text, nullable=True)
    moment_title_language = Column(String, nullable=True)
    moment_title_source = Column(String, nullable=True)
