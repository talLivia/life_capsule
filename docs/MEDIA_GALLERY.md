...
**Written 2026-08-06. Nothing built.** Three related gaps in what a timeline
...
1. Click an empty portrait, or "Add photos" on a period.
...
| Phase | Work | Why here |
| --- | --- | --- |
| **1** | G — recordings as sub-bubbles, people as a filter | No schema, no storage, fixes existing content, and is the surface E and F both hang from |
| **2** | `media_assets` + presign/upload/delete endpoints | The shared foundation |
| **3** | E — entity photo, primary only, on the extraction panel and the tree | Smallest useful slice of the new table |
| **4** | E everywhere else — timeline bubbles, entity list | Pure rendering once the payload carries `photo_url` |
| **5** | F — period galleries and the lightbox | Needs 2, benefits from 3's upload UI |
...
| 4 | Can family accounts (`/talk`) see photos? | Not in this plan. Everything here is producer-side; showing photos to family is a separate call about what the archive exposes. |
...
- Anything about years — see `docs/TIMELINE_YEAR_ATTRIBUTION.md`.
